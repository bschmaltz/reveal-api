class VotesController < ApplicationController
  ...
  def update
    ...
    @post = Post.find(vote_params[:post_id])
    if !@user.nil? and !@vote.nil? and !@post.nil? and @user.id==vote_params[:user_id] and @vote.user_id==@user.id and @post.user_id!=@user.id and @vote.update_attributes(vote_params)
      @result = true
    else
      @result = false
    end
  ...
  def vote_params
    params.require(:vote).permit(:user_id, :post_id, :action)
  end
end
